...
Example:
    ...
    if method=='heatmap':
        if heatmap_summarize is True:
            ...
            k = data.groupby(['phenotype'],observed=True).mean() # collapse the whole dataset into mean expression
            indices = []
            for i in k.index:
                if i in k.columns:
                    indices.append(i)
            indices = pd.CategoricalIndex(indices)
            d = k[indices]
        else:
        ...
        if method=='numeric':
            if x_axis is None and y_axis is None and facet_by is None and plot_type is None:
                ...
                # change col -> hue
                plot = sns.catplot(data=d, x=x_axis, y=y_axis, hue=facet_by, kind=plot_type, **kwargs)
